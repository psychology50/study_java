...
/*
  '21.11.14 무언가 잘못 돌아가는 것 같지만 다시 찾아본 생성자 개념
  String str = new String("자바");
  => 문자열을 처리하는 String Type의 레퍼런스(참조변수)를 선언하고
 	 이름이 str이라는 변수를 메모리 stack 영역에 4byte 크기의 저장공간을 생성한 후,
 	 new 생성자를 이용해 String 참조변수(객체)를 만들어 "자바"값을 힙 영역에 할당한다.
 	 그리고 그 저장공간의 주소값을 stack 영역의 str 변수에 저장하는 것이다.
 	 
  Scanner scanner = new Scanner(); 또한 마찬가지다.
...
public class WhatClass_Constructor {
	
	public static void main(String[] args) {
		new Aclass001(); // 기본 생성자
		
		Aclass001 a = new Aclass001(); // 객체 생성
		// Aclass 객체 값을 담을 수 있는 구조 a의 공간을 new로 만들고 Aclass()로 만든 객체 값을 a에 담아라
